...
function startEnemyTurn(){
  ...
  // На финальной стадии боя (мало ХП у Аксия) он с шансом 60% мешает две
  // атаки одновременно — например яблочный дождь вместе с бластерами.
  if(stage >= 2 && Math.random() < 0.6){
    const options = COMBOABLE.filter(n=>n!==baseName);
    const partner = options[Math.floor(Math.random()*options.length)];
    name = "combo:" + baseName + "+" + partner;
    comboPreset = { names: [baseName, partner], presets: { blasterBottles: { count: blasterLevel } } };
  }
